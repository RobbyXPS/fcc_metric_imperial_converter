...
suite('Unit Tests', function(){
  ...
  suite('Function convertHandler.getNum(input)', function() {
    ...
    test('Invalid Input (double fraction)', function(done) {
      var input = '1/2/3gal';
      assert.equal(convertHandler.getNumError(input),true);
      done();
    });
  ...
  suite('Function convertHandler.getUnit(input)', function() {
    ...
    test('Unknown Unit Input', function(done) {
      var input = '4gala'
      assert.equal(convertHandler.getUnitError(input),true);
      done();
    });  
  ...
  suite('Function convertHandler.spellOutUnit(unit)', function() {
    ...
    test('For Each Valid Unit Inputs', function(done) {
      ...
      input.forEach(function(item,i){
        //console.log('>>>',convertHandler.spellOutUnit(item))
        assert.equal(convertHandler.spellOutUnit(item), expected[i])
      })
      done();
    });
